...
class TurtlesimBridge(Node):
    def __init__(self):
        ...
        self.get_logger().info("Turtlesim Bridge running! Translating game data for the AI.")
    ...
    def timer_cb(self):
        ...
        # Turtlesim grid is 11x11. We map it to 640x480 pixels.
        x_px = int((self.latest_pose.x / 11.0) * 640)
        y_px = int((self.latest_pose.y / 11.0) * 480)
        
        # Draw the turtle as a green circle
        cv2.circle(img, (x_px, 480 - y_px), 20, (0, 255, 0), -1) 
        
        img_msg = self.bridge.cv2_to_imgmsg(img, encoding="bgr8")
        img_msg.header.stamp = self.get_clock().now().to_msg() # Required for message_filters!
